...
class BBGDetail extends StatefulWidget {
  ...
  BBGDetail({this.id});
  @override
  _BBGDetailState createState() => _BBGDetailState(id: id);
}

class _BBGDetailState extends State<BBGDetail> {
  final int id;
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Padding(
                                padding: const EdgeInsets.only(left: 2),
                                child: Text('${snapshot.data.electricalPower}'),
                              ),
                            ),
                          ],
                        ),
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Padding(
                                padding: const EdgeInsets.only(left: 2),
                                child: Text('${snapshot.data.mediaInfo}'),
                              ),
                            ),
                          ],
                        ),
    ...
    return Alert(
      ...
      content: Column(
        children: <Widget>[
          SizedBox(height: 5),
          Text(
            "Anda yakin ingin menghapus pengalihan BBG ini? ",
            style: TextStyle(
  ...
  Future<DetailData> getFutureBBGDetail() async {
    String accessToken = await storageCache.read(key: 'access_token');
    String lang = await storageCache.read(key: 'lang');
    var response = await http.get(
        '${UrlCons.mainDevUrl}customer-service/pengalihan-bbg/${widget.id}',
        headers: {
  ...
  void deleteFormId() async {
    ...
    var responseDeleteCustId = await http.delete(
      '${UrlCons.mainProdUrl}customer-service/pengalihan-bbg/${widget.id}',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $accessToken',
        'Accept-Language': lang
