...
import 'package:pgn_mobile/screens/pengajuan_layanan/penghentian_sementara/penghentian_sementara_update.dart';
import 'package:rflutter_alert/rflutter_alert.dart';

class PenghentianSementaraDetail extends StatefulWidget {
  final int id;
  PenghentianSementaraDetail({this.id});
  @override
  _PenghentianSementaraDetailState createState() =>
      _PenghentianSementaraDetailState(id: id);
}

class _PenghentianSementaraDetailState
    extends State<PenghentianSementaraDetail> {
  final int id;
  _PenghentianSementaraDetailState({this.id});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          'Pengajuan Penghentian Sementara',
          style: TextStyle(color: Colors.white, fontSize: 16),
        ),
      ),
      body: FutureBuilder<DetailPenghentianSementara>(
          future: getFuturePenghentianSementaraDetail(),
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Padding(
                                padding: const EdgeInsets.only(left: 2),
                                child: Text(
                                    '${DateFormat('dd MMMM yyy').format(DateTime.parse(snapshot.data.subDateSuspend).toLocal())}'),
                              ),
                            ),
                          ],
                        ),
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Padding(
                                padding: const EdgeInsets.only(left: 2),
                                child: Text(
                                    '${DateFormat('dd MMMM yyy').format(DateTime.parse(snapshot.data.subDateEnable).toLocal())}'),
                              ),
                            ),
                          ],
                        ),
                        ...
                        child: Row(
                          ...
                          children: [
                            Container(
                              width: 150,
                              child:
                                  Text('Pencabutan Meter Gas yang Terpasang'),
                            ),
                            Container(
                              margin: const EdgeInsets.only(left: 10),
                              width: 10,
                            ...
                            Expanded(
                              child: Padding(
                                padding: const EdgeInsets.only(left: 2),
                                child: Text(snapshot.data.needPlugin == 0
                                    ? 'Tidak'
                                    : 'Ya'),
                              ),
                            ),
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Padding(
                                padding: const EdgeInsets.only(left: 2),
                                child: Text('${snapshot.data.reason}'),
                              ),
                            ),
                          ],
                        ),
    ...
    return Alert(
      ...
      content: Column(
        children: <Widget>[
          SizedBox(height: 5),
          Text(
            "Anda yakin ingin menghapus pengajuan penghentian sementara ini ? ",
            style: TextStyle(
  ...
  Future<DetailPenghentianSementara>
      getFuturePenghentianSementaraDetail() async {
  ...
  void deleteFormId() async {
    ...
    var responseDeleteCustId = await http.delete(
      '${UrlCons.mainProdUrl}customer-service/temporary-suspend/${widget.id}',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $accessToken',
        'Accept-Language': lang
    ...
    CreatePenghentianSementara deleteFormId =
        CreatePenghentianSementara.fromJson(
            json.decode(responseDeleteCustId.body));
