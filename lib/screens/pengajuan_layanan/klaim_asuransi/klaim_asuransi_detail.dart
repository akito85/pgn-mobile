...
class KlaimAsuransiDetail extends StatefulWidget {
  final int id;
  KlaimAsuransiDetail({this.id});
  @override
  _KlaimAsuransiDetailState createState() => _KlaimAsuransiDetailState(id: id);
}

class _KlaimAsuransiDetailState extends State<KlaimAsuransiDetail> {
  final int id;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          'Klaim Asuransi Kebakaran Detail',
          style: TextStyle(color: Colors.white, fontSize: 16),
        ),
      ),
      body: FutureBuilder<DetailData>(
          future: getFutureKlaimAsuransiDetail(),
    ...
    return Alert(
      ...
      content: Column(
        children: <Widget>[
          SizedBox(height: 5),
          Text(
            "Anda yakin ingin menghapus pengajuan klaim asuransi ini ? ",
            style: TextStyle(
  ...
  Future<DetailData> getFutureKlaimAsuransiDetail() async {
    String accessToken = await storageCache.read(key: 'access_token');
    String lang = await storageCache.read(key: 'lang');
    var response = await http.get(
        '${UrlCons.mainDevUrl}customer-service/insurance-claim/${widget.id}',
        headers: {
  ...
  void deleteFormId() async {
    ...
    var responseDeleteCustId = await http.delete(
      '${UrlCons.mainProdUrl}customer-service/insurance-claim/${widget.id}',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $accessToken',
        'Accept-Language': lang
